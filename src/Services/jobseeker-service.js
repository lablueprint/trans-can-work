...
export const createJobseeker = async (email, data) => {
  await setDoc(doc(db, "jobseekers", email), data)
    .then(function () {
...
export const deleteJobseeker = async (email) => {
  await deleteDoc(doc(db, "jobseekers", email))
    .then(() => {
      console.log(
        "Jobseeker account ",
        email,
...
export const pingNavigator = async (
  JSemail,
  milestoneIndex,
  NavEmail,
  milestoneArray
  ...
  await updateDoc(doc(db, "jobseekers", JSemail), {
